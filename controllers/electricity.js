...
// Handle building the view for creating a electricity.
// No body, no in path parameter, no query.
// Does not need to be async
exports.electricity_create_Page = function(req, res) {
    console.log("create view")
    try{
     res.render('electricitycreate', { title: 'Electricity Create'});
    }
    catch(err){
     res.status(500)
     res.send(`{'error': '${err}'}`);
...
// Handle building the view for updating a electricity.
// query provides the id
exports.electricity_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
    let results = await electricity.findById(req.query.id)
    res.render('electricityupdate', { title: 'Electricity Update', toShow: results });
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
exports.electricity_delete_Page = async function(req, res) {
    console.log("Delete view for id " + req.query.id)
    try{
    results = await electricity.findById(req.query.id)
    res.render('electricitydelete', { title: 'Electricity Delete', toShow:
   results });
    }
    catch(err){
    res.status(500)
